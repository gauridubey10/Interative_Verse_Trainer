...
const removeTrailingLetter = () => {
  ...
  if(level == 10 && (algoType == AlgorithmType.FIRSTLETTER || algoType == AlgorithmType.LASTLETTER || algoType == algoType.RANDOMLETTER )){updatedVerse= replaceLettersInParagraph(updatedVerse, false, true); lastLevel = true;}
...
function replaceLettersInParagraph(paragraph, removeAllLetters, removeAllTrailingLetter) {
    ...
    if(algoType == AlgorithmType.RANDOMLETTER){
     ...
     modifiedWords = words.map((word) =>
        replaceRandomLettersInWord(word, removeAllLetters, removeAllTrailingLetter)
    );
    }
    else if(algoType == AlgorithmType.FIRSTLETTER){
       modifiedWords = words.map((word) =>
       replaceStartLettersInWord(word, removeAllLetters, removeAllTrailingLetter)
    );
    }
    else if(algoType == AlgorithmType.LASTLETTER){
       modifiedWords = words.map((word) =>
       replaceLastLettersInWord(word, removeAllLetters, removeAllTrailingLetter)
    );
    }
...
function replaceLastLettersInWord(word, removeAllLetters, removeAllTrailingLetter) {
    let replacedWord = "";
    ...
    for (let j = word.length-1; j >= 0; j--) {
        if (removeAllLetters) {
            ...
            else if (!oneLetterReplaced &&( removeAllTrailingLetter || (letter !== "_"))) {
                replacedWord += "_";
...
function replaceStartLettersInWord(word, removeAllLetters, removeAllTrailingLetter) {
    let replacedWord = "";
    let oneLetterReplaced = false;
    for (let j = 0; j < word.length; j++) {
        if (removeAllLetters) {
            ...
            const letter = word.charAt(j);
            if(removeAllTrailingLetter){
                // console.log(word,"   sjsjjsjssj  ", word[j]);
...
function replaceRandomLettersInWord(word, removeAllLetters, removeAllTrailingLetter) {
